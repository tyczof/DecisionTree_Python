...
class WezelDecyzyjny:
    def __init__(self, attribute=None, decision=None, value=None):
        self.attribute = attribute  # Numer atrybutu używanego w tym węźle
        self.decision = decision  # Decyzja (klasa) przypisana do tego węzła
        self.value = value  # Wartość, która prowadzi do tego węzła
        self.children = {}    # Słownik zawierający potomków tego węzła
...
def klasaDecyzyjna(dane):
    return dane[-1][-1]


def konstruujDrzewo(dataset, node=None, value=None):
    if node is None:
        node = WezelDecyzyjny(value=value)
    ...
    if not stop_condition(gain_ratios):
        ...
        for i, val in enumerate(set(best_attr)):
            ...
            node.children[i] = konstruujDrzewo(child_dataset, value=val)
    else:
        node.decision = klasaDecyzyjna(dataset)  # Ostatni węzeł staje się liściem
        node.value = value  # Przypisanie wartości, która prowadzi do tego liścia
...
def main():
    ...
    dataset = data4
    # Tworzenie drzewa
    drzewo = konstruujDrzewo(dataset)

    with open('tree_visualization.txt', 'w') as f:
        visualize_tree_to_file(drzewo, file=f)
